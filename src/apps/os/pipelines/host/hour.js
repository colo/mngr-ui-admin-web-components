...
const debug = Debug('apps:os:pipelines:host:hour')
...
export default {
  ...
  filters: [
    ...
    // function (doc, opts, next, pipeline) {
    //   debug('MERGE %o %o', doc, buffer, pipeline.get_input_by_id('input.os.host.hour').conn_pollers)
    //
    //   let timeout
    //
    //   let _merge = function () {
    ...
    //
    //   if (buffer.length === pipeline.get_input_by_id('input.os.host.hour').conn_pollers) { // || buffer_expire < Date.now()
    //     _merge()
    //   } else {
    //     buffer.push(doc)
  ...
  output: [
    ...
    //
    //   if (!payload.err) { EventBus.$emit('input.os.host.hour.' + payload.metadata.input, payload) }
    //
    //   // if (!payload.err) { EventBus.$emit('log', payload) }
    // }
    function (payload) {
      if (!payload.err && /^input\.os\.host\.hour\[.*\]$/.test(payload.id)) {
        payload.id = payload.id.replace('input.os.host.hour[', '').slice(0, -1)
        debug('OUTPUT', payload)
        EventBus.$emit('input.os.host.hour.' + payload.metadata.input, payload)
      }

...
const debug = Debug('apps:os:pipelines:hosts:minute')
...
Array.each(IO(), function (io, index) {
  ios.push({
    id: 'input.os.hosts.minute.' + index,
    module: InputIO,
    index: index
  },)
})
export default {
  ...
  filters: [
    ...
    function (doc, opts, next, pipeline) {
      debug('MERGE %o %o', doc, buffer, pipeline.get_input_by_id('input.os.hosts.minute').conn_pollers)
      ...
      if (buffer.length < Object.getLength(pipeline.get_input_by_id('input.os.hosts.minute').conn_pollers)) { buffer.push(Object.clone(doc)) }

      if (buffer.length >= Object.getLength(pipeline.get_input_by_id('input.os.hosts.minute').conn_pollers)) { // || buffer_expire < Date.now()
        _merge()
      }
      // else {
  ...
  output: [
    function (payload) {
      if (!payload.err && /^input\.os\.hosts\.minute\[.*\]$/.test(payload.id)) {
        payload.id = payload.id.replace('input.os.hosts.minute[', '').slice(0, -1)
        debug('OUTPUT', payload)
        EventBus.$emit('input.os.hosts.minute.' + payload.metadata.input, payload)
      }

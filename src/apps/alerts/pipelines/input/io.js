...
import IO from '@etc/alerts.io'

export default new Class({
  Extends: App,

  // types: ['count', 'alerts', 'paths'],
  // recived: [],
  ...
  options: {
    ...
    io: {
			...
			// rooms: ['alerts'], //atomatically join connected sockets to this rooms
			routes: {
				// 'app.doc': [{
				// 	// path: ':param',
  ...
  //
  // },
  alerts: function(socket, next, doc){
    // let {type} = doc
    debug('alerts %o', doc)

    if(doc.status){
      debug('ERROR alerts %o', doc)
    }
    ...
    //   debug('alerts %o', doc)
    //   // Array.each(doc.alerts.tags, function(tag){
    //   //   debug('TAG %s', tag)
    ...
    //   // }.bind(this))
    //   //
    //   // Array.each(doc.alerts.hosts, function(host){
    //   //   debug('HOST %s', host)
    //   //   // this.io.emit('/tags/'+tag)
    //   //   this.io.emit('/', {
    //   //     params: { prop: 'hosts' },
  ...
  //
	// 	// arguments[1]()
	// 	// this.io.to('alerts').emit('response', 'a new user has joined the room saying '+arguments[2]);
	// 	// next(socket)
	// },
  ...
  initialize: function(options){
    ...
    this.add_io(IO()[options.index])
		...
		this.profile('alerts_init');//start profiling
    ...
    this.addEvent('onExit', function(){
      ...
      // if(this.io.disconnected == false)
		...
		this.profile('alerts_init');//end profiling

		this.log('alerts', 'info', 'alerts started');
  },
